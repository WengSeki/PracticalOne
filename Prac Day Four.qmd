...
dist_tbl <- flight1|>group_by(carrier)|> summarise(mean_distance = mean(distance),sd_distance = sd(distance) )
dist_tbl|>arrange(mean_distance)
```

```{r}
flight1|>filter(carrier == "OO")|>select(distance)
flight1|>filter(carrier == "YV")|>select(distance)
```

```{r}
dep_delay_mean <- flights|> group_by(month,carrier)|> summarise(average_delay = mean(dep_delay, na.rm = TRUE),.groups = "drop")
dep_delay_mean|>pivot_wider(names_from = carrier,values_from = average_delay)
```

```{r}
flight_ontime <- flights|>filter(dep_delay>0, arr_delay<=0)|>summarise(proportion = n()/nrow(flights))|> pull(proportion)
flight_ontime
```

```{r}
multi_airline_routes <- flights |> group_by(origin, dest) |> filter(n_distinct(carrier) > 1) |> distinct(origin, dest) |> ungroup()
multi_airline_routes
```

```{r}
route_performance <- flights |> filter(paste(origin, dest) %in% paste(multi_airline_routes$origin, multi_airline_routes$dest)) |> group_by(origin, dest, carrier) |> summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE),
    .groups = "drop"  ) |> left_join(airlines, by = "carrier") |> rename(airline = name)

route_performance
```

```{r}
best_worst <- route_performance |>  group_by(origin, dest) |> filter(n() >= 2) |>  summarise( best_airline = airline[which.min(avg_arr_delay)], worst_airline = airline[which.max(avg_arr_delay)], performance_gap = max(avg_arr_delay) - min(avg_arr_delay), .groups = "drop") |> arrange(desc(performance_gap))
best_worst
```

```{r}
worst_route <- best_worst |> arrange(desc(performance_gap)) >  head(1)
```

```{r}
#| echo: false
my_data <-structure(list(id = c("id_1", "id_2", "id_3", "id_4", "id_5", 
